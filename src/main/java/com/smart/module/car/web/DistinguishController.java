...
import com.smart.module.car.util.VehicleIdentify;
    ...
    public Result upload(MultipartFile file,Long id) {
        try {
            if(file!=null){
                ...
                //OSS存储
                String piUrl = UploadUtil.uploadImage(file);
                log.info("图片存储路径："+piUrl);
                //临时存储到项目，方便车牌识别处理
                file.transferTo(new File(SystemConstant.PIC_PATH));
                //车牌识别
                VehicleIdentify vehicleIdentify = new VehicleIdentify(SystemConstant.PIC_PATH);
                Map<VehicleMessage, String> vehicleCodeMap = vehicleIdentify.getVehicleCode();
                String plateNumber = vehicleCodeMap.get(VehicleMessage.VEHICLE_CODE);
